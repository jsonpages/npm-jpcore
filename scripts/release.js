...
function stepStack() {
  log("Step 2/5: @jsonpages/stack — version patch & publish");
  const dir = path.join(ROOT, "packages", "stack");
  run("npm version patch --no-git-tag-version", dir);
  if (!dryRun) {
...
function stepCore() {
  log("Step 3/5: @jsonpages/core — build, version patch & publish");
...
function stepTenantAlpha(coreVersion) {
  log("Step 4/5: tenant-alpha — pin @jsonpages/core, build & dist");
...
function stepCli() {
  log("Step 5/5: @jsonpages/cli — build, version patch & publish");
