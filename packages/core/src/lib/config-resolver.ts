...
type RefDocuments = NonNullable<JsonPagesConfig['refDocuments']>;
...
function getHeaderDataMenuCandidate(headerData: unknown): MenuItem[] | null {
  ...
  if (Array.isArray(links) && links.every(isMenuItemShape)) {
    return links as MenuItem[];
  }
  const menu = headerData.menu;
  if (Array.isArray(menu) && menu.every(isMenuItemShape)) {
    return menu as MenuItem[];
  }
  const menuConfig = headerData.menuConfig;
  if (Array.isArray(menuConfig) && menuConfig.every(isMenuItemShape)) {
    return menuConfig as MenuItem[];
  }
  return null;
}
